...
Install dotfiles on Linux. Existing dotfiles are not merged or backed up
...
install_packages() {
    ...
    if command -v apt >/dev/null 2>&1; then
        ...
        warn "Unknown package manager. Install manually: zsh tmux git stow vim neovim curl wget"
    fi
}
...
main() {
    if [[ "$(uname -s)" != "Linux" ]]; then
        echo "This script is intended for Linux. Use ./install_macos.sh on macOS."
        exit 1
    fi
